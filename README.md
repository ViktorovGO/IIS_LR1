...
В рамках данного раздела был налажен веб-интерфейс prometheus, существующий для сбора и хранения метрик, генерируемых сервисами. Файл конфигурации и полученные скриншоты хранятся в соответствующей директории /src/prometheus . Доступ к веб-интерфейсу осуществляется по адресу  http://localhost:9090.

В рамках тестирования интерфейса были полученные следующие скриншоты:

Гистограмма предсказаний модели
![Гистограмма предсказаний модели](/src/prometheus/hist.png)

Частота запросов к основному сервису
![Частота запросов к основному сервису](/src/prometheus/rate200.png)

Частота запросов к сервису с кодами ошибок 4** и 5**
![Количество запросов к сервису с кодами ошибок 4** и 5**](/src/prometheus/rate!=200.png)

### Дашбоард

В рамках завершающего этапа были разработан сервис grafana.

Сервис, созданный для визуализации метрик путём создания дашбордов. Полученный в результате создания дашборд хранится в src/grafana. Веб-интерфейс запускается по адресу http://localhost:3000. В качестве database используется сервис prometheus.

Панель
![Panel](/src/grafana/panel.png)

Описание метрик, полученных на дашборде выше:

1. Requests(status=2xx)

Показывает частоту запросов к сервису со статус кодом 2xx

2. Requests(status!=2xx)

Показывает частоту запросов к сервису со статус кодомам не равным 2xx

3. prediction_total_req

Показывает частоту запросов к сервису с предсказаниями

4. Hist of predictions

Показывает гистрограмму с предсказаниями
...
Датчик, показывающий сдвиг входных данных относительно данных, на которых обучалась модель. Может повлиять на точность модели.